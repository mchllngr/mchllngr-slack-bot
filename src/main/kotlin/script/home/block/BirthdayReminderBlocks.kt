...
class BirthdayReminderBlocks(
    ...
    fun createBlocks(user: User?): List<LayoutBlock> {
        val dbUser = user?.id?.let { userRepo.select(UserId(it)) } ?: return emptyList() // TODO really emptyList?
        ...
        return listOf(
            ...
            actions(
                listOf(
                    checkboxes {
                        val areBirthdateRemindersEnabled = dbUser.enableBirthdateReminders
